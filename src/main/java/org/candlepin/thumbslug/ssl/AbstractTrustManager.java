...
public abstract class AbstractTrustManager implements X509TrustManager {
    ...
    protected void checkCertificateChain(X509Certificate[] certs)
        throws CertificateException {
        // work up the client certs chain, verifying the current against the next.
        // once we hit the end, see if it verifies against _any_ of the certs in the ca
        // chain.
        if (certs.length > 1) {
